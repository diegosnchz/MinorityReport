...
# ============================================================================
# GHOST CLIENT - NEO4J ALIGNED (CITIZENS, LOCATIONS, VISIONS)
# ============================================================================
...
class GhostClient:
    def __init__(self, data):
        ...
        for link in self.links:
            ...
            if l_type == 'CONECTA_CON':
                if s not in self.adj_conecta: self.adj_conecta[s] = []
                if t not in self.adj_conecta: self.adj_conecta[t] = []
                self.adj_conecta[s].append(link)
                self.adj_conecta[t].append(link)
            
            # Social links
            elif l_type in ['KNOWS', 'INTERACTS_WITH']:
                if s not in self.social_map: self.social_map[s] = []
        ...
        try:
            if self.is_panicking and self.panic_index < len(self.panic_route):
                ...
                if neighbors:
                    ...
                    sorted_neighbors = sorted(neighbors, key=lambda x: x.get('riesgo', 0.5))
                    link = random.choice(sorted_neighbors[:2]) # Choose from 2 least risky
                    target = link['target'] if link['source'] == self.current_node_id else link['source']
    ...
    def update_ui(self):
        ...
        if status_el:
            ...
            status_el.style.color = '#00ff95' if node['type'] in ['hideout', 'blindspot'] else '#ff3e3e'
        
        threat = self.calculate_threat_level()
        if threat_val_el:
            threat_val_el.innerText = f"{int(threat * 100)}%"
            threat_val_el.style.color = '#ff3e3e' if threat > 0.6 else '#00f2ff'

        window.updateDeckLayers(json.dumps(self.nodes), json.dumps(self.links), self.current_node_id)
...
def trigger_panic(event=None):
    ...
    # Find hideouts
    hideouts = [n['id'] for n in client.nodes if n['type'] == 'hideout' and n['id'] != client.current_node_id]
    if hideouts:
...
def create_bokeh_chart(data):
    ...
    for c in citizens:
        ...
        if score < 0.3: counts[0] += 1
        elif score < 0.6: counts[1] += 1
        else: counts[2] += 1
    
    p = figure(x_range=risk_ranges, height=140, width=280, title="Citizen Risk Distribution",
               toolbar_location=None, tools="", background_fill_color=None,
               border_fill_color=None, outline_line_color=None,
               sizing_mode="fixed")
