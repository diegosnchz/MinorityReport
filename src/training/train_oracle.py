...
from torch_geometric.data import Data, DataLoader  # type: ignore[import]  # noqa: F401
...
def generate_synthetic_city_graph(
    ...
    Args:
        ...
        device: Dispositivo para los tensores (cuda o cpu)
    ...
    police_presence = (torch.rand(num_nodes, device=device) * danger_level)
    x[:, 0] = police_presence

    # Feature 1: Tipo de ubicación (0=calle, 1=callejón, 2=edificio, 3=parque)
    location_type = (torch.randint(
        0, 4, (num_nodes,), device=device).float() / 3.0)
    x[:, 1] = location_type

    # Feature 2: Nivel de iluminación (inverso al nivel policial = más seguro de noche)
    x[:, 2] = 1.0 - (torch.rand(num_nodes, device=device) * 0.5)

    # Feature 3: Densidad de población (más gente = más fácil esconderse)
    x[:, 3] = torch.rand(num_nodes, device=device)

    # Generar aristas
    edge_index = torch.randint(
        0, num_nodes, (2, num_edges), device=device, dtype=torch.long)
    ...
    safety_score = (1.0 - (src_police + dst_police) / 2.0) * \
        ((src_density + dst_density) / 2.0)
...
def train_epoch(
    ...
    # Backward pass
    loss.backward()  # type: ignore[misc]
    optimizer.step()
...
def train_oracle_net(
    ...
    device: Optional[torch.device] = None
) -> Tuple[OracleNet, dict[str, list[float]]]:
    """
    ...
    Args:
        ...
        device: Dispositivo (torch.device o None para auto-detect)
    ...
    # Configurar dispositivo con auto-detect
    if device is None:
        device = get_device()
    ...
    if device.type == 'cuda':
        try:
            ...
            gpu_memory = torch.cuda.get_device_properties(0).total_memory / 1e9  # type: ignore[misc]
            print(f"   GPU: {gpu_name}")
            print(f"   Memory: {gpu_memory:.2f} GB")

            # Nota sobre NVIDIA Quadro K4200
            if gpu_name and ("K4200" in gpu_name or "Quadro" in gpu_name):
                print(f"\n   [INFO] NVIDIA Quadro K4200 detectada!")
                print(
                    f"   Esta GPU es excelente para entrenar modelos GNN de tamaño medio.")
                print(
                    f"   Con 4GB VRAM, podemos procesar grafos de hasta ~200-300 nodos eficientemente.")
        except Exception as e:
            print(f"   [WARNING] Could not get GPU details: {e}")
    else:
        print(f"   [WARNING] GPU no disponible. Entrenando en CPU.")
    ...
    print(
        f"Final validation accuracy: {training_history['val_accuracy'][-1]:.4f}")
...
def test_trained_model():
    ...
    try:
        checkpoint = torch.load('oracle_net_best.pth', map_location=device)  # type: ignore[misc]
        model.load_state_dict(checkpoint['model_state_dict'])
        print(f"\n[SUCCESS] Loaded trained model (Epoch {checkpoint['epoch']}, "
    ...
    x_test, edge_index_test, _ = generate_synthetic_city_graph(
        num_nodes=50,
        num_edges=150,
        num_features=16,
    ...
    print(
        f"   Risky routes (0.3-0.7): {np.sum((safety_scores >= 0.3) & (safety_scores <= 0.7))}")
